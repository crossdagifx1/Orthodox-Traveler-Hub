...
import { CHURCH_FIELDS } from "@/components/admin/entity-fields";
  ...
  return (
    <div className="p-4 pb-24 bg-background min-h-full">
      ...
      <EntityFormSheet
        ...
        title="New church"
        description="Add an Orthodox church to the world map."
        submitLabel="Create"
        fields={CHURCH_FIELDS}
        isPending={createChurch.isPending}
        onSubmit={async (values) => {
          await createChurch.mutateAsync({ data: values as any });
          await queryClient.invalidateQueries({ queryKey: getListChurchesQueryKey({}) });
          toast({ title: "Church created" });
        }}
      />
