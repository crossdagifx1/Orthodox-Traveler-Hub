...
import { MEZMUR_FIELDS } from "@/components/admin/entity-fields";
...
export function AdminMezmurs() {
  ...
  const createMezmur = useCreateMezmur();
  ...
  return (
    <div className="p-4 pb-24 bg-background min-h-full">
      ...
      <EntityFormSheet
        ...
        title="New mezmur"
        description="Add a hymn or spiritual song."
        submitLabel="Create"
        fields={MEZMUR_FIELDS}
        isPending={createMezmur.isPending}
        onSubmit={async (values) => {
          await createMezmur.mutateAsync({ data: values as any });
          await queryClient.invalidateQueries({ queryKey: getListMezmursQueryKey({}) });
          toast({ title: "Mezmur created" });
        }}
      />
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          (Array.isArray(mezmurs) ? mezmurs : []).map(mezmur => (
