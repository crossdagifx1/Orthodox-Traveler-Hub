...
type CalendarValue = {
  ...
  fastingKey: FastingKey;
  saint: { name: string; description: string };
  feastTitle: string | null;
};

const CalendarContext = createContext<CalendarValue | null>(null);

export function EthiopianCalendarProvider({ children }: { children: ReactNode }) {
  const { i18n } = useTranslation();
  const lang = (i18n.language?.split("-")[0] as "am" | "en") || "am";
  ...
  const value = useMemo<CalendarValue>(() => {
    ...
    return {
      ...
      fastingKey,
      saint: {
        name: saintEntry.name[lang],
        description: saintEntry.description[lang],
      },
      feastTitle: SAINTS_BY_DAY[eth.day] ? saintEntry.name[lang] : null,
    };
  }, [lang]);
...
export function useEthiopianCalendar() {
  ...
  if (!ctx) throw new Error("useEthiopianCalendar must be used within EthiopianCalendarProvider");
  return ctx;
}
